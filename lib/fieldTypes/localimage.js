...
localimage.prototype.uploadImage = function(item, file, data, update, callback) {
	...
	var doSave = function(callback) {
		...
		var nameJPG = name.substr(0, name.lastIndexOf('.'))+"_"+uuid.v4()+".jpg";
		...
		console.log('Cropping: '+file.path+" "+path.join(field.options.dest, nameJPG)+" "+img.width+" "+img.height+" "+img.x+" "+img.y);
		easyimg.crop({
